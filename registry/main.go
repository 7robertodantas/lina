...
func (s *RegistryService) RegisterDevice(c *gin.Context) {
	...
	query := `
		INSERT OR REPLACE INTO devices (id, public_key, unit, price_per_unit, secret_key)
		VALUES (?, ?, ?, ?, ?)
	`
	_, err := s.db.Exec(query, d.ID, d.PublicKey, d.Unit, d.PricePerUnit, d.SecretKey)
	if err != nil {
...
func (s *RegistryService) GetDeviceConfig(c *gin.Context) {
	...
	var d Device
	query := `SELECT id, public_key, unit, price_per_unit, secret_key FROM devices WHERE id = ? LIMIT 1`
	err := s.db.QueryRow(query, deviceID).Scan(&d.ID, &d.PublicKey, &d.Unit, &d.PricePerUnit, &d.SecretKey)
