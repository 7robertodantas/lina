...
```bash
docker compose -f deployment/docker-compose.edge.yml logs -f -t --no-color device ledger lightning consumption |& tee load-evaluation-edge-node.log
``` -->
...
```bash
docker compose -f deployment/docker-compose.external.yml logs -f -t --no-color httpdevices |& tee load-evaluation-external-machine.log
```
